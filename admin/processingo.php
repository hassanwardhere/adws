...
$stmt = $pdo->prepare("UPDATE `ordero` SET status = 'Processing' WHERE id = ?");
...
sendStatusChangeEmail($userEmail, 'Processing');

// Redirect back to order details page
header("Location: processordero.php?alertType=success&alertMessage=Package Set to Processing successfully");
exit();
