...
if (isset($_POST['checkout'])) {
    ...
    $insertOrderStmt = $pdo->prepare("INSERT INTO orders (packageid, userid, cartid, packagename, additionaldetails, device, price, discount, totalpaid, validity, status, namesoncreditcard, creditcardnumber, expiredate, csvnumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
function sendOrderPlacementEmail($email)
{
    ...
    try {
        //Server settings
        $mail->isSMTP();
        $mail->Host       = 'agencyhub.tayoinc.com'; // SMTP server
        $mail->SMTPAuth   = true;
        $mail->Username   = 'newcargo@agencyhub.tayoinc.com'; // SMTP username
        $mail->Password   = 'Hassan135790,.';     // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Enable TLS encryption
        $mail->Port       = 587;  // TCP port to connect to

        //Recipients
        $mail->setFrom('newcargo@tayoinc.com', 'You Placed an Order');
